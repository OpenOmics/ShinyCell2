...
wrShFunc <- function() {
  glue::glue(
    ...
    '}} \n',
    '\n',
    '# User-controllable background colour for a plot and its legend. \n',
    '# Defaults to white so every plot looks identical regardless of UI theme. \n',
    '# Applied as a theme layer appended after the plot is built; the same fill is \n',
    '# carried into standalone legends extracted via g_legend(). \n',
    'scBg <- function(col = "white"){{ \n',
    '  if(is.null(col) || is.na(col) || col == "") col <- "white" \n',
    '  theme( \n',
    '    plot.background       = element_rect(fill = col, colour = NA), \n',
    '    panel.background      = element_rect(fill = col, colour = NA), \n',
    '    legend.background     = element_rect(fill = col, colour = NA), \n',
    '    legend.box.background = element_rect(fill = col, colour = NA), \n',
    '    legend.key            = element_rect(fill = col, colour = NA) \n',
    '  ) \n',
    '}} \n',
    '\n',
    '\n',
    ...
    '  # expm1() can overflow to Inf on large log-values; log1p(mean(Inf)) stays Inf \n',
    '  # and later breaks colour-scale limits ("\'to\' must be a finite number") and \n',
    '  # clustering. Clamp any non-finite aggregate to the finite data range. \n',
    '  if(any(!is.finite(ggData$val))){{ \n',
    '    finVal = ggData$val[is.finite(ggData$val)] \n',
    '    capVal = if(length(finVal)) max(abs(finVal)) else 0 \n',
    '    ggData[val ==  Inf]$val =  capVal \n',
    '    ggData[val == -Inf]$val = -capVal \n',
    '    ggData[is.nan(val)]$val = 0 \n',
    '  }} \n',
    '   \n',
    '  # Drop zero-variance genes. A gene whose mean expression is identical across \n',
    '  # every group carries no information for a heatmap/bubbleplot, and scale() \n',
    '  # would return NaN for it - which then makes dist()/hclust() fail with \n',
    '  # "NA/NaN/Inf in foreign function call". Rather than impute, drop these rows \n',
    '  # and report them back to the user (see noVarGenes attribute below). \n',
    '  geneVar = ggData[, .(v = var(val)), by = "geneName"] \n',
    '  noVarGenes = geneVar[is.na(v) | v == 0]$geneName \n',
    '  if(length(noVarGenes)){{ \n',
    '    ggData = ggData[!geneName %in% noVarGenes] \n',
    '    geneList = geneList[!gene %in% noVarGenes] \n',
    '  }} \n',
    '  shiny::validate(need(uniqueN(ggData$geneName) > 1, \n',
    '    paste0("After dropping genes with no variance across the selected groups (", \n',
    '           paste0(noVarGenes, collapse = ", "), \n',
    '           "), fewer than 2 genes remain. Unable to plot!"))) \n',
    '   \n',
    '  # Scale if required \n',
    ...
    '    }}  \n',
    '  }} \n',
    '  # Carry the dropped zero-variance genes back so the UI can inform the user. \n',
    '  attr(ggOut, "noVarGenes") = noVarGenes \n',
    '  return(ggOut) \n',
    '}} \n',
    '\n',
    ...
    '      legend.text = element_text(angle = 45, hjust = 1),\n',
    '      # Breathing room so the panel edges and the angled colourbar labels at\n',
    '      # the bottom are not clipped inside the fixed-height plot container.\n',
