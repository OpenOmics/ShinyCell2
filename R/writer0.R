...
wrShFunc <- function() {
  glue::glue(
    ...
    '  ggData$sub = inpMeta[, inpConf[UI == inpsub1]$ID, with = FALSE][[1]]\n',
    '  if(inpDtyp == "Cell Information"){{\n',
    '    ggData$val = inpMeta[, inpConf[UI == inp1]$ID, with = FALSE][[1]]\n',
    '  }} else {{\n',
    ...
    '  ggData$sub = inpMeta[, inpConf[UI == inpsub1]$ID, with = FALSE][[1]] \n',
    '  if(inpDtyp == "Cell Information"){{ \n',
    '    ggData$group = inpMeta[, inpConf[UI == inp1]$ID, with = FALSE][[1]] \n',
    '  }} else {{ \n',
    '    h5file <- H5File$new(inpH5, mode = "r") \n',
    '    h5data <- h5file[["grp"]][["data"]] \n',
    ...
    '  \n',
    '  if(image_type == "VisiumV2") {{\n',
    '    # Apply 90° CW rotation + Y-flip for VisiumV2\n',
    '    x_center <- mean(range(ggData$X))\n',
    '    y_center <- mean(range(ggData$Y))\n',
    '    x_centered <- ggData$X - x_center\n',
    '    y_centered <- ggData$Y - y_center\n',
    '    # 90° CW: (x, y) -> (y, -x)\n',
    '    x_rotated <- y_centered\n',
    '    y_rotated <- -x_centered\n',
    '    # Place at image center\n',
    '    ggData$X <- x_rotated + (img_w / 2)\n',
    '    ggData$Y <- y_rotated + (img_h / 2)\n',
    '    # Apply Y-flip\n',
    '    ggData$Y <- img_h - ggData$Y\n',
    '  }} else {{\n',
    '    # VisiumV1: No transformation - display raw coordinates for testing\n',
    '  }}\n',
    '  \n',
    ...
    '  \n',
    '  # Match spatial coordinates with metadata using cellID\n',
    '  spatial_cells <- rownames(inpImg$coord)\n',
    '  meta_subset <- inpMeta[cellID %in% spatial_cells]\n',
    '  # Ensure order matches by setting key\n',
    '  setkey(meta_subset, cellID)\n',
    '  ggData$cellID <- spatial_cells\n',
    '  setkey(ggData, cellID)\n',
    '  ggData$sub = meta_subset[ggData, on = "cellID"][[inpConf[UI == inpsub1]$ID]]\n',
    '  ggData$siz = 1\n',
    '  if(inpDtyp == "Cell Information"){{\n',
    '    ggData$val = meta_subset[ggData, on = "cellID"][[inpConf[UI == inp1]$ID]]\n',
    '  }} else {{\n',
    '    h5file <- H5File$new(inpH5, mode = "r") \n',
    '    h5data <- h5file[["grp"]][["data"]] \n',
    '    # Get indices of spatial cells in the full dataset, preserving order\n',
