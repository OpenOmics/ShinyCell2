...
wrShFunc <- function() {
  glue::glue(
    ...
    '  subset_type,\n',
    '  subset) {\n',
    '  if (!("factor" %in% names(sc1def))) {\n',
    '    sc1conf$factor <- sapply(sc1conf$UI, function(col_name) {\n',
    '      is.factor(sc1def[[col_name]])\n',
    '    })\n',
    '  }\n',
    '  pcs <- colnames(sc1dimr[[selected_dimr]])\n',
    '  stopifnot(length(pcs) == 2) # assert two PCs\n',
    '  ggData <- as.data.frame(sc1dimr[[selected_dimr]])\n',
    '  names(ggData) <- c("X", "Y")\n',
    '  ggData$split <- sc1meta[[splitby]]\n',
    '  if (gene_exp == "") {\n',
    '    gene_exp <- sc1def$genes[[selected_assay]][1]\n',
    '  }\n',
    '  gene_id <- sc1gene[[selected_assay]][[gene_exp]]\n',
    '  h5in <- paste0("sc1assay_", selected_assay, ".h5")\n',
    '  h5file <- H5File$new(h5in, mode = "r")\n',
    '  h5data <- h5file[["grp"]][["data"]]\n',
    '  gene_data <- h5data$read(args = list(gene_id, quote(expr = )))\n',
    '  h5file$close_all()\n',
    '  ggData$gene <- gene_data\n',
    '  ggData[ggData$gene < 0]$gene <- 0\n',
    '  if (length(subset) != 0 && !is.null(subset_type) && subset_type %in% names(sc1meta)) {\n',
    '    subset_mask <- sc1meta[[subset_type]] %in% subset\n',
